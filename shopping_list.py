...
def parser_shopping_list(shopping_list, day):   #new
    parsered_shopping_list = []
    for ingredien in shopping_list["aisles"][1]["items"]:
        parsered_ingredien = parser_ingredien(ingredien)
        parsered_shopping_list.append(parsered_ingredien)
    parsered_shopping_list.append({"total cost": shopping_list["cost"]})
    with open(f'mealplans/{day}/shopping_list.json', 'w') as file:
        json.dump(parsered_shopping_list, file)


def parser_ingredien(ingredien):    #new
    parsered_ingredien = {}
